                   ...
                   int index, pup_segment* segment, int additional)
{
  ...
  if (read != 1)
  {
    printf("Failed to read segment #%d for verification! %d\n",
                 index, read);
    result = -1;
    goto end;
  }
                    ...
                    uint16_t index, pup_segment* segment)
{
  ...
  if (remaining_size > 0)
  {
    ...
    if (read != 1)
    {
      printf("Failed to read segment #%d! #%d\n", index, read);
      result = -1;
      goto end;
    }
    ...
    if (result != 0)
    {
      printf("Failed to decrypt segment #%d! %d\n", index, errno);
      goto end;
    }
                           ...
                           uint16_t table_index, pup_segment* table_segment)
{
  ...
  if (read != 1)
  {
    printf("  Failed to read table for segment #%d! %d\n", index, read);
    result = -1;
    goto end;
  }
  ...
  if (result != 0)
  {
    printf("  Failed to decrypt table for segment #%d! %d\n",
                 index, errno);
    goto end;
  }
  ...
  for (int i = 0; i < block_count; i++)
  {
    ...
    if (result < 0)
    {
      printf("  Failed to decrypt block for segment #%d! %d\n",
                   index, errno);
      goto end;
    }
...
int decrypt_pup_data(const decrypt_state state)
{
  ...
  if (read != 1)
  {
    printf("Failed to read file header! (%u)\n", read);
    goto end;
  }
  ...
  if (read != 1)
  {
    printf("Failed to read header! (%u)\n", read);
    goto end;
  }
  ...
  if ((file_header.flags & 1) == 0)
  {
    ...
    if (result != 0)
    {
      printf("Failed to decrypt header! %d\n", errno);
      goto end;
    }
  }
  else
...
void decrypt_pups()
{
  ...
  if (read != 1)
  {
    printf("Failed to read BLS header!\n");
    goto end;
  }
  ...
  {
    printf("Failed to read BLS entries!\n");
    goto end;
  }
